...
description: Machine-actionable error codes emitted during agent execution.
---

When invoked with `--json`, error responses include machine-readable error codes. Autonomous agents inspect these codes to take self-correcting actions.

| Error Code | Root Cause | Automated Agent Remediation |
| :--- | :--- | :--- |
| `ERR_LOW_MEMORY` | Available system RAM is below the safety threshold ($< 2.5\text{GB}$). | 1. Advise user of host memory pressure.<br>2. Switch from 6-stem to 4-stem model.<br>3. Close background processes or retry on larger instance. |
